...
static void Cmd_getexp(void)
{
        ...
        if (gBattleControllerExecFlags == 0)
        {
             ...
             || GetMonData(&gPlayerParty[*expMonId], MON_DATA_SPECIES_OR_EGG) == SPECIES_EGG)
            {
                ...
                gBattleMoveDamage = 0; // used for exp
                  ...
                  || GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) == MAX_LEVEL || levelCapped(GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL)))
            {
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_HP) && (GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) != MAX_LEVEL || !levelCapped(GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL))))
            {
